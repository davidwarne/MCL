...
/*model simulation routine: See Tanaka et al. for details*/
int simulate_model(void *sim, SSAL_real_t * theta, Dataset * D_s)
{
    ...
    gH = (double*)D_s->fields[0].data_array;

    /*initialise  model*/
    X = (unsigned int *)malloc(N_stop*sizeof(unsigned int));
    memset(X,0,N_stop*sizeof(unsigned int));
    ...
    /* sub-sample population and compute diversity metric */
    for (i=0;i<(N - n);i++)
    {
