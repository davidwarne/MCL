...
int dabcrjs(ABC_Parameters abc_p_c, ABC_Parameters abc_p_f,Dataset *data, CDF_estimate *F_c, double *theta_c, double *theta_f, double * rho_f)
{
    ...
    p_c = (double*)malloc(F_c->G.numPoints*sizeof(double));

    p_c_s = cdf2pmf(F_c,p_c);
    /*allocate memory for simulated  dataset*/
    data_s = copyDataset(data); 
    j = 0;
    while (j < abc_p_f.nacc)
    {
        /*sample from previous distributions*/
        J = durngpmfs(F_c->G.numPoints,p_c,p_c_s);
        for (k=0;k<abc_p_f.k;k++)
        {
            theta_f[j*abc_p_f.k + k] = F_c->G.coords[J*F_c->G.dim + k] + durngus(-1.0,0.0)*F_c->G.deltas[k];
        }
        /*simulate process*/
        (*(abc_p_f.s))(abc_p_f.sim,theta_f+j*abc_p_f.k,data_s);
        /*accept/reject based of rho*/
        d = (*(abc_p_f.rho))(data,data_s);
        if (d <= abc_p_f.eps)
        {
            if (rho_f != NULL)
                rho_f[j] = d;
